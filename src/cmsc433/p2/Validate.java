...
public class Validate {
	...
	public static boolean validateSimulation(List<SimulationEvent> events,
			int numCustomers, int numCooks, int numTables, int machineCapacity) {
		boolean foundError = true;
		foundError = check(
				events.get(0).event == SimulationEvent.EventType.SimulationStarting,
				"Simulation didn't start with initiation event")
				&& foundError;
		foundError = check(
				events.get(events.size() - 1).event == SimulationEvent.EventType.SimulationEnded,
				"Simulation didn't end with termination event")
				&& foundError;
		foundError = check(customerEnteredBeforePlacing(events, numCustomers),
				"Customer ordered before entering restaurant") && foundError;
		foundError = check(restaurantCapacityCheck(events, numTables),
				"Number of customers exceeded number of tables") && foundError;
		foundError = machineCapacityCheck(events, machineCapacity)
				&& foundError;
		foundError = check(customerReceivedBeforeLeaving(events, numCustomers),
				"customer received order before cook finished") && foundError;
		foundError = check(customerLeaveTest(events, numCustomers),
				"customer left before receiving order") && foundError;
		foundError = check(
				cookCompletesBeforeCustomerReceives(events, numCustomers),
				"customer received order before cook completed") && foundError;
		foundError = check(machineFinishesFoodBeforeCookFinishesFood(events, numCustomers),
				"customer left before receiving order") && foundError;
		foundError = check(allCustomersServed(events, numCustomers),
				"all customers were not served") && foundError;
		  foundError = check(customerBeforeCookOrders(events),
	                "Cook received an order before a customer placed it.") && foundError;
	        foundError = check(cookTakesOrderBeforeCooksStartsFood(events),
	                "Cook started food before cook took the order") && foundError;
	        foundError = check(cookStartsFoodBeforeMachine(events),
	                "A machine started cooking a food item before a cook started that item")
	                && foundError;
	        foundError = check(machineStartsFoodBeforeFinishes(events),
	                "A machine finished a food item before the machine started that item")
	                && foundError;
	        foundError = check(cookFinishesFoodsBeforeCompletesOrder(events),
	                "A cook finished an order before the cook finished all the food in that order")
	                && foundError;
		return foundError;
	...
	private static boolean restaurantCapacityCheck(
	...
	private static boolean machineCapacityCheck(List<SimulationEvent> events,
		...
		return check(resultBurgers, "Exceeded grill capacity")
				&& check(resultFries, "Exceeded fryer capacity")
				&& check(resultCoffees, "Exceeded coffee maker capacity");
	}

	private static boolean customerEnteredBeforePlacing(
			List<SimulationEvent> events, int numCustomers) {
	...
	private static boolean machineFinishesFoodBeforeCookFinishesFood(
		...
		for (SimulationEvent e : events) {
			if (e.event == EventType.CookFinishedFood) {
				...
				if (numBurgersTaken > numBurgersFinished
						|| numFriesTaken > numFriesFinished
				...
				if (e.food==FoodType.burger) {
					...
					throw new IllegalStateException("Invalid machine name: "
	...
	private static boolean customerReceivedBeforeLeaving(
			List<SimulationEvent> events, int numCustomers) {
	...
	private static boolean allCustomersServed(List<SimulationEvent> events,
			int numCustomers) {
		boolean result = true;
    ...
    /**
     * Tests that cooks dont take orders before customers place them
     * 
     * @param events
     * @return
     */
     ...
     * @return
     */
    private static boolean cookTakesOrderBeforeCooksStartsFood(
     ...
     * @return
     */
    private static boolean machineStartsFoodBeforeFinishes(
            List<SimulationEvent> events) {
        boolean passed = true;
     ...
     * @return
     */
    private static boolean cookFinishesFoodsBeforeCompletesOrder(
            List<SimulationEvent> events) {
        boolean passed = true;
        HashSet<Integer> set = new HashSet<Integer>();
